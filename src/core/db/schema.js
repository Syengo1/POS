...
export const productSchema = {
  ...
  properties: {
    id: { type: 'string', maxLength: 100 },
    sku: { type: 'string' }, // Essential for Shisha/Nicotine variants
    name: { type: 'string' },
    ...
    fraction_of_parent: { type: 'number' }, 
    ...
    items: {
      ...
      items: {
        ...
        properties: {
          ...
          promotion_id: { type: 'string' } // Which promo triggered the discount
        },
        required: ['product_id', 'name', 'qty', 'base_price', 'final_price']
      }
    },
    total_amount: { type: 'number' }, // Final KES paid
    total_discount: { type: 'number' }, // Total KES saved by customer
    payment_method: { type: 'string' }, // 'CASH' or 'MPESA'
    mpesa_ref: { type: 'string' }, 
